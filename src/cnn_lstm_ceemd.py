...
save_dir = os.path.join(current_dir,"ceemd_cnn_lstm")
...
for symbol in symbols:
    ...
    ceemd_imfs = apply_decomposition(values, method='ceemd')
    ...
    for i in tqdm(range(ceemd_imfs.shape[0])):
        ...
        cnn_lstm_model = CNN_LSTM(input_size=1,cnn_hidden_output=cnn_hidden_output,lstm_hidden_size=lstm_hidden_size).to(device)
        train_model(cnn_lstm_model, train_dl, test_dl, epochs=train_imf_epochs[i], lr=1e-4)
        save_final_path = os.path.join(save_dir,f"cnn_lstm_imf_{i+1}.pth")
...
results_df.to_csv("results_ceemd_cnn_lstm.csv")
